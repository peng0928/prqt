...
def get_datetime_now(rule='%Y-%m-%d %H:%M:%S', reduce=None):
    daytime = datetime.datetime.now()
    end_time = datetime.datetime.strptime(reduce, rule)
    if reduce:
        daytime = (daytime - end_time)

    else:
        daytime = daytime.strftime('%H:%M:%S')
